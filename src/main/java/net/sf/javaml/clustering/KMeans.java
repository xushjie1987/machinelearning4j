...
/**
 * Implements the K-means algorithms as described by Mac Queen in 1967. <bibtex> J. B. MacQueen
 * (1967): "Some Methods for classification and Analysis of Multivariate Observations, Proceedings
 * of 5-th Berkeley Symposium on Mathematical Statistics and Probability", Berkeley, University of
 * California Press, 1:281-297 </bibtex>
 * 
 * @author Thomas Abeel
 */
public class KMeans implements Clusterer {
    ...
    /**
     * The number of iterations the algorithm should make. If this value is Integer.INFINITY, then
     * the algorithm runs until the centroids no longer change.
     */
    ...
    /**
     * Constuct a default K-means clusterer with 100 iterations, 4 clusters, a default random
     * generator and using the Euclidean distance.
     */
    public KMeans() {
        this(4);
    }
    ...
    /**
     * Constuct a default K-means clusterer with the specified number of clusters, 100 iterations, a
    ...
    /**
     * Create a new Simple K-means clusterer with the given number of clusters and iterations. The
     * internal random generator is a new one based upon the current system time. For the distance
     * we use the Euclidean n-space distance.
     * 
     * @param clusters
     *            the number of clusters
     * @param iterations
    ...
    /**
     * Create a new K-means clusterer with the given number of clusters and iterations. Also the
     * Random Generator for the clusterer is given as parameter.
     * 
     * @param clusters
    ...
    public Dataset[] cluster(Dataset data) {
        ...
        int instanceLength = data.instance(0)
                                 .noAttributes();
        for (int j = 0; j < numberOfClusters; j++) {
            ...
            //
            // }
            double[] randomInstance = DatasetTools.getRandomInstance(data,
                                                                     rg);
        ...
        while (randomCentroids || (iterationCount < this.numberOfIterations && centroidsChanged)) {
            ...
            for (int i = 0; i < data.size(); i++) {
                int tmpCluster = 0;
                double minDistance = dm.measure(centroids[0],
                                                data.instance(i));
                for (int j = 1; j < centroids.length; j++) {
                    double dist = dm.measure(centroids[j],
                                             data.instance(i));
                    if (dm.compare(dist,
                                   minDistance)) {
                        minDistance = dist;
